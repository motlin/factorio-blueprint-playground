...
async function findMatchingCompression(blueprintString: string): Promise<{
    ...
    for (const settings of generateCompressionSettings()) {
        const key = `level=${settings.level} wb=${settings.windowBits} mem=${settings.memLevel} strat=${settings.strategy}`
        attempted.add(key)
        ...
        try {
            ...
            failed.add(`${key} (error: ${error.message})`)
        }
    }
...
describe('compression settings search', () => {
    it('finds exact boundaries of working compression settings', async () => {
        ...
        const {matches} = await findMatchingCompression(minimal)
