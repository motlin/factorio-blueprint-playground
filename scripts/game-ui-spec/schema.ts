...
const gameUiSpecSchema = z.object({
	...
	styles: z.object({
		...
		buttonHorizontalPadding: z.number().int().nonnegative(),
		buttonMinimalHeight: z.number().int().positive(),
		buttonMinimalWidth: z.number().int().positive(),
		draggableSpaceHeaderLeftMargin: z.number().int().nonnegative(),
		frameActionButtonSize: z.number().int().positive(),
		framePaddingBottom: z.number().int().nonnegative(),
		framePaddingLeft: z.number().int().nonnegative(),
		framePaddingRight: z.number().int().nonnegative(),
		framePaddingTop: z.number().int().nonnegative(),
		frameTitleFontSize: z.number().int().positive(),
		headerFillerHeight: z.number().int().positive(),
		scrollbarThumbWidth: z.number().int().positive(),
		scrollbarWidth: z.number().int().positive(),
		tooltipFrameOpacity: z.number().positive().max(1),
		tooltipLabelMaximalWidth: z.number().int().positive(),
		tooltipLabelMinimalWidth: z.number().int().positive(),
		bindings: z.object({
